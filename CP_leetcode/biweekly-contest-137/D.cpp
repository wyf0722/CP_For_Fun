    ...
    long long maximumValueSum(vector<vector<int>>& board) {
        ...
        auto update = [&](vector<int> &row) -> void {
            for (int j = 0; j < n; j++) {
                ...
                if (val > p[0].first) {
                    ...
                    p[0] = make_pair(val, j);
                } else if (val > p[1].first && j != p[0].second) {
                    if (j != p[1].second) {
                        p[2] = p[1];
                    }
                    p[1] = make_pair(val, j);
                } else if (val > p[2].first && j != p[0].second && j != p[1].second) {
                    p[2] = make_pair(val, j);
                }
            }
        };

        vector<vpii> pre(m, vpii(3, make_pair(-inf, -1))), suf(m, vpii(3, make_pair(-inf, -1)));
        for (int i = 0; i < m - 2; i++) {
            update(board[i]);
            pre[i] = p;
        }
        fill(all(p), make_pair(-inf, -1));
        for (int i = m - 1; i > 1; i--) {
            update(board[i]);
            suf[i] = p;
        }
        ...
        for (int i2 = 1; i2 < m - 1; i2++) {
            for (int j2 = 0; j2 < n; j2++) {
                for (auto &[val1, j1] : pre[i2 - 1]) {
                    for (auto &[val3, j3] : suf[i2 + 1]) {
                        if (j1 != j2 && j2 != j3 && j1 != j3) {
                            chmax(ans, 1LL * val1 + val3 + board[i2][j2]);
                        }
                    }
                }
            }
