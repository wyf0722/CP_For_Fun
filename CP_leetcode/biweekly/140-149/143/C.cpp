...
public:
    int maxFrequency(vector<int>& nums, int k, int numOperations) {
        ...
        for (int x : st) {
            // [x - k, x + k]
            int cnt_x = mp[x];
            int r_x = upper_bound(nums.begin(), nums.end(), x + k) - upper_bound(nums.begin(), nums.end(), x);
            int l_x = lower_bound(nums.begin(), nums.end(), x) - lower_bound(nums.begin(), nums.end(), x - k);
            ans = max(ans, cnt_x + min(numOperations, l_x + r_x));
        }
        return ans;
    }
};