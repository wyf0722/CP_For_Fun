...
class Solution {
public:
    int f(vector<int> a_st, vector<int> a_du, vector<int> b_st, vector<int> b_du) {
        int mn_finish = inf;
        for (int i = 0; i < a_st.size(); i++) {
            chmin(mn_finish, a_st[i] + a_du[i]);
        }

        int res = inf;
        for (int i = 0; i < b_st.size(); i++) {
            chmin(res, max(mn_finish, b_st[i]) + b_du[i]);
        }
        return res;
    }
