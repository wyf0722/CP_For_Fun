...
public:
    int minTimeToReach(vector<vector<int>>& moveTime) {
        ...
        min_heap<array<int, 3>> q;
        vector<vector<vector<int>>> dis(n, vector<vector<int>>(m, vector<int>(2, inf)));
        dis[0][0][0] = 0;
        q.push({0, 0, 0});
        while (!q.empty()) {
            auto [cur_d, t, pos] = q.top();
            q.pop();
            int x = pos / m, y = pos % m;
            if (cur_d > dis[x][y][t]) continue;
            for (int d = 0; d < 4; d++) {
                int nx = x + dx[d], ny = y + dy[d];
                if (0 <= nx && nx < n && 0 <= ny && ny < m) {
                    int nxt_d = max(moveTime[nx][ny], cur_d) + (t == 0 ? 1 : 2);
                    if (nxt_d < dis[nx][ny][t ^ 1]) {
                        dis[nx][ny][t ^ 1] = nxt_d;
                        q.push({nxt_d, t ^ 1, nx * m + ny});
                    }
                }
            }
        }
