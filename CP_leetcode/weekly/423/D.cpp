...
public:
    int countKReducibleNumbers(string s, int k) {
        ...
        auto dfs = [&](auto &&self, int i, int one, bool is_limit) -> int {
            if (i == n) {
                return !is_limit && one == 0 ? 1 : 0;
            }
            if (!is_limit && mem.at(i).at(one) != -1) {
                return mem.at(i).at(one);
            }
            int res = 0;
            int up = is_limit ? s.at(i) - '0' : 1;
            // fill 0
            res += self(self, i + 1, one, is_limit && 0 == up);
            res %= MOD;
            // fill 1
            if (one && up) {
                res += self(self, i + 1, one - 1, is_limit && 1 == up);
                res %= MOD;
            }
            if (!is_limit) mem.at(i).at(one) = res;
            return res;
        };

        vector<int> f(n + 1);
        for (int i = 1; i <= n; i++) {
            f.at(i) = f.at(__builtin_popcount(i)) + 1;
            if (f[i] <= k) {
