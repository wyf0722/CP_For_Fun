...
public:
    EventManager(vector<vector<int>>& events) {
        for (auto &e : events) {
            ...
            id2priority[id] = priority;
            h.emplace(priority, -id);
        }
    }
    
    void updatePriority(int eventId, int newPriority) {
        id2priority[eventId] = newPriority;
        h.emplace(newPriority, -eventId);
    }
    
    int pollHighest() {
        while (!h.empty()) {
            auto [priority, id] = h.top();
            h.pop();
            id = -id;
            if (id2priority[id] != priority) {
                continue;
            }
            id2priority[id] = -1;
            return id;
