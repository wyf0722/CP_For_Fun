...
public:
    vector<int> minDeletions(string s, vector<vector<int>>& queries) {
        ...
        vector<int> vals;
        for (int i = 0; i < n; i++) {
            init.push_back(Info{s[i] - 'A', s[i] - 'A', 0});
            vals.push_back(s[i] - 'A');
        }
        vector<int> ans;
        ...
        for (auto &q : queries) {
            if (q[0] == 1) {
                vals[q[1]] ^= 1;
                seg.modify(q[1], Info{vals[q[1]], vals[q[1]], 0});
            } else {
                ans.push_back(seg.rangeQuery(q[1], q[2] + 1).op);
            }
        }
        return ans;
    }
