...
public:
    int minValidStrings(vector<string>& words, string target) {
        ...
        int mx_w = 0;
        for (auto &w : words) {
            mx_w = max(mx_w, (int)w.size());
        }
        vector<unordered_set<i64>> hash_code(mx_w + 1);
        for (auto &w : words) {
            StringHash sh(w);
            mx_w = max(mx_w, (int)w.size());
            for (int i = 0; i < w.size(); i++) {
                hash_code[i + 1].insert(sh.query(1, i + 1));
            }
        }
        StringHash sh(target);
        auto get = [&](int st) -> int {
            int lo = 0, hi = min(mx_w, n - st) + 1;
            while (lo + 1 < hi) {
                int mid = (lo + hi) / 2;
                i64 target_code = sh.query(st + 1, st + mid);
                if (hash_code[mid].count(target_code)) {
                    lo = mid;
                } else {
                    hi = mid;
                }
