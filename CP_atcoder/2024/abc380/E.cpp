...
void solve() {
    int n, q;
    cin >> n >> q;
    vector<int> color(n + 1), cnt(n + 1, 1);
    ...
    while (q--) {
        ...
        if (op == 1) {
            ...
            cin >> x >> c;
            int L = dsu.find(x);
            int cur_color_size = dsu.size(x);
            // 修改当前段
            cnt[color[L]] -= cur_color_size;
            color[L] = c;
            cnt[color[L]] += cur_color_size;
            // left段
            if (color[dsu.find(L - 1)] == c) {
                dsu.merge(L - 1, L);
            }
            // right段
            if (color[dsu.find(L + cur_color_size)] == c) {
                dsu.merge(L, L + cur_color_size);
            }
        } else {
            int c;
            cin >> c;
