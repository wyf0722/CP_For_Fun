...
void solve() {
    ...
    for (int i = 0; i < n;) {
        ...
        if (block == k - 1) segs.emplace_back(st, i - 1);
        if (block == k) segs.emplace_back(st, i - 1);
    }
    int cnt1 = segs[1].second - segs[1].first + 1;
    for (int i = segs[0].second + 1; i <= segs[0].second + cnt1; i++) {
        s[i] = '1';
    }
    for (int i = segs[0].second + cnt1 + 1; i <= segs[1].second; i++) {
        s[i] = '0';
    }
    cout << s << endl;
}
