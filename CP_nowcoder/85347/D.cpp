...
void solve() {
    ...
    q.emplace(0, 0, a[0][0] % p);
    dis[0][0][a[0][0] % p] = 0;
    while (!q.empty()) {
        auto [x, y, cur_mod] = q.front();
        q.pop();
        FOR(d, 0, 4) {
            int nx = x + dx[d];
            int ny = y + dy[d];
            if (nx < 0 || nx >= n || ny < 0 || ny >= m) continue;
            int nxt_mod = (cur_mod + a[nx][ny]) % p;
            if (dis[nx][ny][nxt_mod] < 0) {
                dis[nx][ny][nxt_mod] = dis[x][y][cur_mod] + 1;
                q.emplace(nx, ny, nxt_mod);
            }
        }
    }
    cout << dis[n - 1][m - 1][0] << endl;
}
