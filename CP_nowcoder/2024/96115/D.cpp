...
void solve() {
    ...
    V<V<V<int>>> plans(q, V<V<int>>(n, V<int>(m)));
    ...
    int ans = -1;
    for (int s = 0; s < (1 << q); s++) {
        if (__builtin_popcount(ans) > __builtin_popcount(s)) {
            vector<vector<int>> ck(n, vector<int>(m));
            for (int bit = 0; bit < q; bit++) {
                if (s >> bit & 1) {
                    for (int i = 0; i < n; i++) {
                        for (int j = 0; j < m; j++) {
                            ck[i][j] |= plans[bit][i][j];
                        }
                    }
                }
            }
    ...
    if (ans == -1) cout << -1 << endl;
        ...
        vector<int> choose;
        for (int i = 0; i < q; i++) if (ans >> i & 1) choose.push_back(i + 1);
