...
void solve() {
    int n;
    cin >> n;
    vector<vector<i64>> pre(3, vector<i64>(n + 1));
    FOR(i, 0, 3) {
        FOR(j, 0, n) {
            cin >> pre[i][j + 1];
            pre[i][j + 1] += pre[i][j];
    ...
    i64 tar = (pre[0][n] + 2) / 3;
    vector<int> ans(6, -1);
    for (auto p : vector<vector<int>>{{0, 1, 2}, {0, 2, 1}, {1, 0, 2}, {1, 2, 0}, {2, 0, 1}, {2, 1, 0}}) {
        int i = p[0], j = p[1], k = p[2];
        // [0, l] >= tar
        int l = fstTrue(0, n - 1, [&](auto &x) { return pre[i][x + 1] >= tar; });
        // [r, n - 1] >= tar
        int r = lstTrue(0, n - 1, [&](auto &x) { return pre[k][n] - pre[k][x + 1] >= tar; }) + 1;
        // [l + 1, r - 1]
        if (pre[j][r] - pre[j][l + 1] >= tar) {
            ans[i * 2] = 1;
            ans[i * 2 + 1] = l + 1;
            ans[j * 2] = l + 2;
            ans[j * 2 + 1] = r;
