...
void solve() {
    ...
    vector<array<i64, 2>> st;
    for (int i = 0; i < n; i++) {
        i64 total = a[i];
        i64 piles = 1;
        while (!st.empty()) {
            auto [lst_total, lst_piles] = st.back();
            if (lst_total / lst_piles < total / piles) {
                break;
            }
            total += lst_total;
            piles += lst_piles;
        ...
        st.push_back({total, piles});
    }
    // 上取整
    auto [total1, piles1] = st.back();
    // 下取整
    auto [total2, piles2] = st[0];
    cout << (total1 + piles1- 1) / piles1 - total2 / piles2 << "\n";
}
