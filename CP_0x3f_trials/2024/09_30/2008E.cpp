...
void solve() {
    ...
    if (n % 2 == 0) {
        ...
        for (int i = 0; i < 26; i++) mx_even = max(mx_even, suf[0][i]);
        for (int i = 0; i < 26; i++) mx_odd = max(mx_odd, suf[1][i]);
        cout << n - mx_even - mx_odd << "\n";
        return;
    }
    // 为奇数必须删除一个字母
    int mx = 0;
    vector<array<int, 26>> pre(2);
    for (int i = 0; i < n; i++) {
        suf[i % 2][s[i] - 'a']--;
        int mx1 = 0, mx2 = 0;
        for (int j = 0; j < 26; j++) {
            // 新数组的偶数位 = 偶数位 + 奇数位
            mx1 = max(mx1, pre[0][j] + suf[1][j]);
            // 新数组的奇数位 = 奇数位 + 偶数位
            mx2 = max(mx2, pre[1][j] + suf[0][j]);
        }
        mx = max(mx, mx1 + mx2);
        pre[i % 2][s[i] - 'a']++;
    }
