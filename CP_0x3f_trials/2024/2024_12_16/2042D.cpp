...
void solve() {
    ...
    Fenwick<Info> fen(n);
    for (int i = 0; i < n; i++) {
        int l = segs[i][0], r = segs[i][1], idx = r_idx[r];
        if (!(i + 1 < n && segs[i + 1][0] == l && segs[i + 1][1] == r)) {
            auto info = fen.sum(idx + 1);
            int res = info.minR - info.maxL;
            if (res < 2e9) {
                ans[segs[i][2]] = res - (r - l);
            }
        }
        fen.add(idx, Info{l, r});
    }
