...
void solve() {
    ...
    vector<int> left_s(26, -1), right_t(26, -1);
    ...
    for (int i = 0; i < 26; i++) {
        if (left_s[i] != -1 && right_t[i] != -1) {
            int len = get_len(left_s[i], right_t[i]);
            if (l == -1 || len < get_len(l, r)) {
                l = left_s[i];
                r = right_t[i];
            }
        }
    }
    cout << (l == -1 ? "-1" : s.substr(0, l + 1) + t.substr(r + 1)) << endl;
}
