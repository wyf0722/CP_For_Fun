/*************************************************************************
    > File Name: P_1725_琪露诺.cpp
    > Author: Wu YiFan
    > Github: https://github.com/wyf0722
    > Mail: wyf0722chosen@gmail.com
    > Created Time: 2025-04-16 12:44
************************************************************************/
#include <bits/stdc++.h>
...
void solve() {
    ...
    seg.modify(0, Info{0});
    for (int i = 1; i <= n + 1; i++) {
        seg.modify(i, Info{-inf});
    }
    for (int i = 0; i <= n; i++) {
        int L = min(i + l, n + 1);
        int R = min(i + r, n + 1);
        int fi = seg.rangeQuery(i, i + 1).x;
        if (fi == -inf) {
            continue;
        }
        seg.rangeApply(L, R + 1, Tag{fi + a[i]});
    }
    int res = seg.rangeQuery(n + 1, n + 2).x;
    cout << (res == -inf ? -1 : res) << '\n';
}
